...
# pgrep treats leading "--" as flags; use a flag-safe pattern.
NN_PGREP_PATTERN='astex_nonnative_298K|benchmark astex_nonnative'
...
wait_for_astex_nonnative_clear() {
  ...
  local interval=120
  local elapsed=0
  while pgrep -fl "$NN_PGREP_PATTERN" >/dev/null 2>&1; do
    ...
    pgrep -fl "$NN_PGREP_PATTERN" | head -3 || true
    sleep "$interval"
    elapsed=$((elapsed + interval))
  done
...
if pgrep -fl "$NN_PGREP_PATTERN" >/dev/null 2>&1; then
