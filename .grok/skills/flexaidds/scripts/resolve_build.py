...
def sync_flexaidds_env(resolution: BuildResolution) -> Path:
    ...
    if not results:
        icloud = preserved.get("FLEXAIDDS_ICLOUD")
        if icloud:
            # Prefer FLEXAIDDS_ICLOUD for results location (iCloud Drive by default for safety)
            results = str(Path(icloud).expanduser() / "results")
        else:
            results = str(
                Path.home()
                / "Library/Mobile Documents/com~apple~CloudDocs/flexaidds/results"
            )
