...
def _build_mode(mode_id: int, poses: Sequence[PoseResult], rank: int = 1) -> BindingModeResult:
    ...
    return BindingModeResult(
        ...
        cf_com=best_pose.cf_com if best_pose else None,
        cf_wal=best_pose.cf_wal if best_pose else None,
        top_contact_pair=best_pose.top_contact_pair if best_pose else None,
        top_contact_energy=best_pose.top_contact_energy if best_pose else None,
...
def load_results(path: str | Path) -> DockingResult:
    ...
    modes_with_ranks = [
        BindingModeResult(
            ...
            cf_com=m.cf_com,
            cf_wal=m.cf_wal,
            top_contact_pair=m.top_contact_pair,
            top_contact_energy=m.top_contact_energy,
