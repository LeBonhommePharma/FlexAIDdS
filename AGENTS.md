...
- **DoF search budget: modulate population (chromosomes), not generations.** Base CLI is often `--ga-population 1000 --ga-generations 6000`, but claim runs must keep **generations fixed** and scale **pop** via `FLEXAIDDS_EVAL_SCALE_DIHEDRAL=1` (and optional `FLEXAIDDS_BUDGET_SCALE` for high-DoF). Mode `0` (gen-scale) is legacy; mode `-1` (fixed pop+gen) is oracle-ceiling only. Never “match” a campaign by freezing both axes because an earlier run printed 1000×6000 — that was the **base**, not the effective budget (see `[EVAL-BUDGET]` logs).
...
- **Grand-canonical competitive binding (Ξ / GPF).** For multi-ligand or concentration-dependent work (via --conc, YAML `ligand_concs` / `competition_sets`, or TargetConfig), always register real ensemble log_Z (from BindingPopulation.get_global_ensemble() / StatMech compute or equivalent) into TargetServer + GrandPartitionFunction. Single-ligand canonical paths (no TargetServer / default 1 M only) must remain bit-identical to pre-GPF behavior. Validate p_bind, log_Xi, selectivity, and occupancy against fixtures (grand_synthetic/, competition_example.yaml) before claiming. Use grand_summary CSV/JSON + REMARK GRAND outputs. Preserve log-space numerics and z = c/1 M fugacity.
