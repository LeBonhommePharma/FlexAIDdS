...
5. **Scientific Reviewer + Reproducibility Agent**: Inspects the generated image against the metadata:
   - Correct numeric values in the right colored boxes (cyan TΔS, purple -TΔS prominent, gold I_E-E developed in skill; no -ΔH/-dH boxes or labels per user preference).
