...
| Successful poses (RMSD_hungarian < 2.0 Å) | **68 / 85 (80.0% with validity fallback min(valid hungarian, valid cluster) for reported pose on proper 7r+THERMO reviewer trees + two-stage + G_bind; restores published rate with data fix for sentinel RMSDs in h)** |
