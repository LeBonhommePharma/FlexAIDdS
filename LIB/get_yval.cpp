...
const Lut& lut_for(energy_matrix* em, int bins)
{
	std::lock_guard<std::mutex> lock(g_mu);
	Lut& lut = g_luts[em];
	if (lut.bins == bins && (int)lut.y.size() == bins) return lut;
	lut.bins = bins;
	lut.y.resize(static_cast<size_t>(bins));
	const double denom = static_cast<double>(bins - 1);
	for (int i = 0; i < bins; ++i) {
		const double ra = static_cast<double>(i) / denom;
		lut.y[static_cast<size_t>(i)] = get_yval_scan(em, ra);
	}
	return lut;
}
...
double get_yval(struct energy_matrix* em, double relative_area)
{
	...
	const Lut& lut = lut_for(em, bins);
	const double t = relative_area * static_cast<double>(bins - 1);
	int i = static_cast<int>(t);
	if (i < 0) i = 0;
	if (i >= bins - 1) return lut.y.back();
	const double f = t - static_cast<double>(i);
	return lut.y[static_cast<size_t>(i)] * (1.0 - f)
	     + lut.y[static_cast<size_t>(i + 1)] * f;
}
