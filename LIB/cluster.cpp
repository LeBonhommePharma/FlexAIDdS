...
void cluster(FA_Global* FA, GB_Global* GB, VC_Global* VC, chromosome* chrom, genlim* gene_lim, atom* atoms, resid* residue, gridpoint* cleftgrid, int num_chrom, char* end_strfile, char* tmp_end_strfile, char* dockinp, char* gainp, target::TargetServer* ts, const std::string& ligand_name)
{
	...
	for(j=0;j<num_of_results;++j)
	{
		...
		// EMITTED ONCE PER POSE, OUTSIDE THE optres LOOP, AND READ OFF THE
		// AGGREGATE cf. Both of those are corrections to the first version of this
		// edit:
		//   (1) OBJECT. compute_ligand_dsvib() (ic2cf.cpp) populates the aggregate
		//       cfstr that ic2cf() RETURNS -- the `cf` assigned above. The optres
		//       loop reads &FA->optres[i].cf, a DIFFERENT object that no dS_vib code
		//       ever writes, so every component read there was a default 0 and
		//       dsvib_status was 0 even in runs where the term demonstrably
		//       executed. Measured on the canary: 0 of 35 ON-arm poses carried a
		//       DSVIB line despite [DSVIB-INIT] appearing in the log.
		//   (2) CARDINALITY. dS_vib is one quantity per LIGAND, not per optimizable
		//       residue. Inside the loop it would be emitted num_optres times.
		// The placement also removes the duplicate-REMARK hazard at its root: this
		// block no longer sits between the gist_desolv cat and a later pre-existing
		// cat, so it cannot re-flush a stale tmpremark. Measured on the canary
		// binary: 37 of 38 gate-OFF poses carried REMARK CF.gist_desolv TWICE,
		// against 0 of 39 poses on the pre-feature binary 74edbea3.
		// Written whenever the gate produced a status, including the refusals: a row
		// that says status=2 (rigid ligand, dS_vib = 0 by physics) or status=3
		// (mode-count mismatch, refused) is information, whereas an absent line is
		// indistinguishable from a build without the feature.
		// Units are spelled out on every line because the engine's OTHER entropies
		// (h_rep, cf.entropy) are information measures in nats/bits and these are
		// thermodynamic quantities in kcal/mol -- mixing them is the exact error
		// this term exists to remove.
		if (cf.dsvib_status != 0) {
			snprintf(tmpremark, MAX_REMARK,
			         "REMARK DSVIB.status=%d basis=torsional "
			         "n_torsion_dofs=%d n_flexbonds=%d "
			         "units_S=kcal/mol/K units_G=kcal/mol\n",
			         cf.dsvib_status, cf.dsvib_ndof, cf.dsvib_nflex);
			safe_remark_cat(remark,tmpremark,&remark_len);
			snprintf(tmpremark, MAX_REMARK,
			...
			safe_remark_cat(remark,tmpremark,&remark_len);
			// Not a computed quantity: identically absent in the rigid-receptor limit
			// (every receptor internal coordinate is frozen, so it contributes no
			// mode to either the complex or the apo spectrum). Emitted explicitly so
			// the cycle is auditable from the file.
			snprintf(tmpremark, MAX_REMARK,
