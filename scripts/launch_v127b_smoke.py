#!/usr/bin/env python3
# launch_v127b_smoke.py — 3-target regression ablation (logsumexp-only binary)
#
# Targets: 1G9V, 1SG0, 1TW6 (v127 failures that were v124 successes).
# Binary: a4056163 (Option B logsumexp) WITHOUT ba5364d3 H-bond/VCT patch.
# Protocol: identical to v127 (r0=4, rotamer off, consensus ON, oracle-ceiling).
#
# Compare against:
#   v127 @ ba5364d3 (HEAD) — results in v127_20260629_0139_optB_full85
#   v124 @ 15b536f8 — results in v124_full85_20260626_0413_consensus_guard
#
...
WORKTREE = f"{REPO}/../FlexAIDdS_v127b_repro"
BUILD = f"{WORKTREE}/build_lto"
BINARY_SRC = f"{BUILD}/FlexAIDdS"
BINARY = "/tmp/FlexAIDdS_v127b"
...
def ensure_build(skip_build: bool) -> None:
    ...
    if all(os.path.exists(p) for p in need):
        return
    if skip_build:
        sys.exit(f"ERROR: missing v127b build under {BUILD} (run build_v127b_logsumexp.sh)")
    print(f"Building v127b via {BUILD_SCRIPT}")
...
def ref_snapshot(pdb: str) -> dict:
    import csv
    snap = {}
    for label, run_dir in [("v124", REF_V124_DIR), ("v127", REF_V127_DIR)]:
...
def main():
    ...
    print("\nLaunching v127b smoke-3 — logsumexp-only @ a4056163 (no H-bond/VCT patch)")
    print(f"  targets  : {SMOKE_TARGETS}")
    print(f"  output   : {output}")
    print(f"  threads  : {bench_threads} (concurrent OK — v128 may be running)")
    ...
    prov_doc = {
        "version":       "v127b_smoke3_logsumexp",
        "launched_at":   datetime.datetime.now(datetime.UTC).isoformat().replace("+00:00", "Z"),
        "git_commit":    GIT_COMMIT,
        "worktree":        WORKTREE,
        "description": (
            "Regression ablation: 1G9V, 1SG0, 1TW6 with logsumexp-only binary (a4056163) "
            "without ba5364d3 H-bond cosine gate / C.ar stacking. Protocol matches v127. "
            "Hypothesis: v127 regressions are scoring-kernel not selector."
        ),
        ...
        "audit_notes": {
            "ablation_removes": "ba5364d3 H-bond/VCT patch",
            "ablation_keeps": "a4056163 logsumexp boltzmann_composite",
            "gate": "recover v124-like cf_native / success on 1G9V+1SG0; 1TW6 may need self-peptide handling",
        },
