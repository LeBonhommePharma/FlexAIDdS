#!/usr/bin/env python3
# launch_v125.py — Option A: gate FLEXAIDDS_CONSENSUS_SCORER=0
#
...
# consensus override block explicitly disabled.
#
# Diagnostic goal: if consensus is the culprit, this should recover ~88.1%.
# If score ~ v124 (76%), the consensus block is not the primary cause.
#
...
OUTPUT = f"{RESULTS_DIR}/v125_{TAG}_consensus_off"
CACHE  = f"{RESULTS_DIR}/cache_v125_consensus_off"
...
# Environment — Option A: FLEXAIDDS_CONSENSUS_SCORER=0
env = dict(os.environ)
env.update({
    ...
    "FLEXAIDDS_CONSENSUS_SCORER":      "0",   # <<< Option A: consensus OFF
...
# Explicitly unset anything that could re-enable consensus or add noise
for k in (
...
if __name__ == "__main__":
    ...
    print("\nLaunching v125 — Option A: consensus scorer OFF")
    print(f"  output   : {OUTPUT}")
    print(f"  cache    : {CACHE}")
    print(f"  threads  : {BENCH_THREADS}")
    print(f"  restarts : 5")
    print(f"  consensus: OFF  (FLEXAIDDS_CONSENSUS_SCORER=0)")
    print(f"  rotamer  : OFF")
    print(f"  hbond_s  : ON  (hardcoded v123+)")

    child_pid = launch_session_isolated(cmd, env, OUTPUT, cwd=REPO)

    prov = {
        "version":       "v125_consensus_off",
        "launched_at":   datetime.datetime.utcnow().isoformat() + "Z",
        "git_commit":    git_commit,
        "description": (
            "Option A diagnostic: FLEXAIDDS_CONSENSUS_SCORER=0. "
            "All other knobs identical to v124 (5-restart, SMFREE, r0=4, "
            "hbond_search ON, no rotamer, sas_weight=1.0, seed_elitism=1). "
            "If consensus is the primary culprit of entropy being net-negative, "
            "this should recover LP's 88.1% no-entropy baseline. "
            "If score ~76%, consensus is not the cause and Option B "
            "(log-space logsumexp free energy) is needed."
        ),
        "binary":         BINARY,
        ...
        "env_snapshot": {
            k: env[k] for k in (
                ...
                "FLEXAIDDS_SEED_ELITISM", "FLEXAIDDS_N_ELITE",
                "FLEXAIDDS_BUDGET_SCALE", "FLEXAIDDS_SOFTCORE_WAL",
                "FLEXAIDDS_SOFTCORE_FLOOR", "FLEXAIDDS_T_HOT",
                "FLEXAIDDS_NATIVE_SEED_FRAC", "FLEXAIDDS_DATA_DIR",
            )
        },
    }
    ...
    print(f"\nv125 launched (commit={git_commit})")
