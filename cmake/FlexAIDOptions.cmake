...
if(MSVC)
    option(FLEXAIDS_USE_OPENMP  "Enable OpenMP thread parallelism (MSVC: legacy v2.0)" OFF)
else()
...
function(flexaids_configure_simd target_name)
    if(MSVC)
        if(FLEXAIDS_IS_X86)
            if(FLEXAIDS_USE_AVX512 AND FLEXAIDS_MSVC_SUPPORTS_AVX512)
            ...
            elseif(FLEXAIDS_USE_AVX2 AND FLEXAIDS_MSVC_SUPPORTS_AVX2)
                ...
                message(STATUS "${target_name}: MSVC x86 build — scalar SIMD fallback")
            endif()
        elseif(FLEXAIDS_IS_ARM)
            target_compile_definitions(${target_name} PRIVATE
                FLEXAIDS_USE_NEON FLEXAIDS_SIMD_NEON)
            message(STATUS "${target_name}: Windows ARM64 NEON enabled (baseline ISA)")
        else()
            message(STATUS "${target_name}: non-x86 MSVC target detected — scalar fallback")
        endif()
